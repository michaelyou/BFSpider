...
"""
    具体的线程实现，不停的从TreadPool的任务队列中取出任务执行
"""
...
class Spider(object):
    ...
    def GetPage(self, url):
        ...
        try: 
            ...
            print 'url %s is unreachable. Exception %s %s' % (url, e.__class__.__name__, e)
            self.logger.error(
    ...
    def crawl_page(self, url, depth):
        ...
        print 'crawing %s, the depth is %d' % (url, depth)
        if depth <= 1:# if depth is done then stop
            ...
            if self.parse_html(result):
                #print 'get the bitch'
                print '\033[1;31;40m'
                print '%s is exactly what you are looking for' % url
                print '\033[0m'
    ...
    def start(self):
        ...
        try:
            ...
            self.pool.joinall()#清空队列里的任务，安全的结束线程
...
def main(): 
    ...
    parser.add_option("-d", "--destination", dest="destination", default="http://www.seu.edu.cn/" , help="The destination spider will go")
    parser.add_option("-m", "--maxsize", type="int",dest="maxsize", default=1000, help="Resource's maxsize of spider get,default 100")
    parser.add_option("-t", "--thread", type="int",dest="thread", default=10, help='parallel thread to grab data,default 10')
    parser.add_option("-p", "--depth", type="int",dest="depth", default=5, help='depth in [1, 6) for spider,default 5')
